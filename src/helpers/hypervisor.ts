...
import { getOrCreateHypervisor, getOrCreatePool, getOrCreateToken } from "./entities";
...
export function updateTvl(
  ...
  const tvl0USD = hypervisor.tvl0.toBigDecimal().times(token0.priceUSD).div(exponentToBigDecimal(token0.decimals))
  const tvl1USD = hypervisor.tvl1.toBigDecimal().times(token1.priceUSD).div(exponentToBigDecimal(token1.decimals))
